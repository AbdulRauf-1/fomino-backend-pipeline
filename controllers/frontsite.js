...
async function emailChecker(req, res) {
  ...
  if (existUser) {
    return res.json(ApiResponse("1", "Email Exist ", "", {}));
  } else if (!existUser) {
    return res.json(ApiResponse("2", "Email not Exist ", "", {}));
  }
}
/*
     1: Register User
...
async function registerUser(req, res) {
  ...
  if (userExist) {
    if (email === userExist.email && signedFrom !== "google" && signedFrom !== "facebook" && signedFrom !== "apple.com") {
      ...
      const data = {
        ...
        joinedOn: userExist.dataValues.joinedOn ? userExist.dataValues.joinedOn : "2023",
        phoneNum: `${userExist.countryCode} ${userExist.phoneNum}`,
      }
      return res.json(returnFunction("1", "Login Successfully!", data, ""));
    } else if (email === userExist.email && signedFrom === "facebook") {
      const accessToken = sign({
        id: userExist.id,
        email: userExist.email,
        deviceToken: "deviceToken",
      ...
      return res.json(returnFunction("1", "Login Successfully!", data, ""));
    } else if (email === userExist.email && signedFrom == "apple.com") {
      const accessToken = sign({
        id: userExist.id,
        email: userExist.email,
        deviceToken: "deviceToken",
  ...
  if (signedFrom === "email") {
    ...
    const userCredit = await Credit.findOne({
      where: {
        referalCode: credit.referalCode
      },
    });
    if (userCredit) {
    ...
    const response = ApiResponse("2", "User registered successfully!", "", data);
    return res.json(response);
    ...
    const data = {
      ...
      email: `${userData.email}`,
      accessToken: `${accessToken}`,
      joinedOn: userData.dataValues.joinedOn ? userData.dataValues.joinedOn : "2023",
      phoneNum: `${userData.countryCode} ${userData.phoneNum}`,
    }
    return res.json(returnFunction("3", "Login Successfully!", data, ""));
  }
}
/*
...
async function verifyOTP(req, res) {
  ...
  if (!userData) {
    return res.json(ApiResponse("1", "User not found!", "", {}));
  }
...
async function signInUser(req, res) {
  try {
    ...
    if (existUser && signedFrom === "google") {
      return res.json(ApiResponse("0", "Try to SignIn by Google ", "", {}));
    }
...
async function forgetPasswordRequest(req, res) {
  ...
  if (!userData) {
    const response = ApiResponse("0", "No User/Driver/Retailer exists against the provided email", "Please sign up first", {});
    return res.json(response);
  }
  let OTP = totp.generate();
  await transporter.sendMail({
...
async function logout(req, res) {
  redis_Client.hDel(`fom${req.user.id}`, "deviceToken").then((upData) => {
    const response = ApiResponse("1", "Logout successfully!", "", {});
    return res.json(response);
  }).catch((err) => {
    const response = ApiResponse("0", "Internal server error", "There is some error logging out. Please try again", {});
    return res.json(response);
  });
}
//! Module:  Home
...
async function home(req, res) {
  ...
  const restaurants = await restaurant.findAll({
    ...
    attributes: ["id", "businessName", "businessEmail", "businessType", "city", "zipCode", "isOpen", "address", "logo", "image", "approxDeliveryTime", "deliveryFeeFixed", "openingTime", "closingTime", "lat", "lng", "isFeatured", ],
  });
...
async function home1(req, res) {
      ...
      validZones.map(async (data) => {
        for (var i = 0; i < data.zoneRestaurants.length; i++) {
          let obj = {
            ...
            rating: "3.7",
            deliveryTime: data?.zoneRestaurants[i]?.restaurant?.approxDeliveryTime,
            deliveryFee: data?.zoneRestaurants[i]?.restaurant?.deliveryFeeFixed,
          };
          if (restaurantId.id == data?.zoneRestaurants[i]?.restaurant?.businessType) {
            if (data?.zoneRestaurants[i]?.restaurant?.isFeatured) popularRestaurants.push(obj);
            restaurantList.push(obj);
          } else if (data?.zoneRestaurants[i]?.restaurant?.businessType == storeId.id) {
            if (data?.zoneRestaurants[i]?.restaurant?.isFeatured) popularStores.push(obj);
            storeList.push(obj);
          }
        }
      });
  ...
  const restaurants = await restaurant.findAll({
    ...
    attributes: ["id", "businessName", "businessEmail", "businessType", "city", "zipCode", "address", "logo", "image", "approxDeliveryTime", "deliveryFeeFixed", "openingTime", "closingTime", "lat", "lng", "isFeatured", ],
  });
  const storeId = await orderApplication.findOne({
    where: {
      name: "store"
      ...
      order.create({
        ...
        orderItems.bulkCreate(products).then((orderItemData) => {
          ...
          orderAddOns.bulkCreate(aoArr).then(async () => {
            ...
            if (deliveryTypeId === "1") {
              if (tmpDistance <= parseFloat(driverbaseCharge.value)) driverEarnings = parseFloat(driverbaseCharge.amount);
              else {
                ...
                driverEarnings = parseFloat(driverbaseCharge.amount) + extraUnits * parseFloat(driverPerMileCharge.amount);
              }
            }
            let adminCommissionPercent = restaurantData.comission;
            let totalEarnings = parseFloat(subTotal) + parseFloat(deliveryFees) + parseFloat(VAT);
            let adminEarnings = totalEarnings * (parseFloat(adminCommissionPercent) / 100);
        ...
        for (var i = 0; i < data.zoneRestaurants.length; i++) {
          if (data.zoneRestaurants[i].restaurant) {
            ...
            let obj = {
              ...
              lat: data?.zoneRestaurants[i]?.restaurant?.lat,
              lng: data?.zoneRestaurants[i]?.restaurant?.lng,
              rating: averageRating != null ? Number(averageRating).toFixed(1).toString() : "0.0",
...
async function getProductDetails(rpId) {
  ...
  try {
    ...
    //   return res.json(productData);
    let currencySign = zonedetails.zone.zoneDetail?.currencyUnit?.symbol ?? "USD";
...
async function getRestaurantById(req, res) {
  ...
  try {
    ...
    let retObj = {
      ...
      deliveryRadius: restaurantData?.distanceUnitID?.symbol === "km" ? restaurantData.deliveryRadius * 1000 : restaurantData.deliveryRadius * 1609,
    };
...
//! Module: Payments
async function makePaymentByNewCard(cardNum, exp_month, exp_year, cvc, amount, orderId, userId,
  // stripeCustomerId,
  // deviceToken,
  isCredit) {
  ...
  if (isCredit) {
    if (parseInt(credit?.point) >= parseInt(amount)) {
      ...
      singleNotification(userData.deviceToken, "Payment Successfully Done", `You have completed the payment of Order ID: ${orderData.id}`);
...
async function createOrder(req, res) {
  ...
  if (parseInt(distance) < 5) {
    ...
    let additionalCharges = parseFloat(additionalDistance) * parseFloat(zoneData?.zone?.zoneDetail?.perKmCharges);
    distanceCharges = parseFloat(first5KmCharge) + parseFloat(additionalCharges);
  }
  let ON1 = uON.generate();
  var cultery_list = [];
  const chargesData = await charge.findAll();
  ...
  const orderData = await order.create({
    ...
    distance,
    subTotal,
    total: parseFloat(total) + parseFloat(zoneCharges) + parseFloat(distanceCharges),
    status: true,
    orderTypeId: type.id,
    dropOffId,
    deliveryTypeId,
...
async function bookTableBooking(req, res) {
  ...
  if (restData) {
    singleNotification(restData?.user?.deviceToken, "Request for Book a Table", `${userData.firstName} ${userData.lastName} make a request for book a table having no of members are ${noOfMembers}`);
  }
...
function categorizeProducts(data) {
  ...
  const storeProducts = [];
  data.forEach(item => {
    const product = item.R_PLink || {};
    const businessType = product.R_MCLink?.restaurant?.businessType;
    if (businessType == "1") {
      restaurantProducts.push(product);
...
async function getWishList(req, res) {
  const ww = await wishList.findAll({
    attributes: ['status'],
    where: [{
      status: true
    }, {
  ...
  const response = ApiResponse("1", "Wish List", "", categorizeProducts(ww));
  return res.json(response);
}
...
async function favRestaurant(req, res) {
  ...
  let storeList = [];
  let all = [];
  let restData = await favouriteRestaurant.findAll({
    where: {
      userId: userId
    },
    include: [{
  ...
  for (var i = 0; i < restData.length; i++) {
    if (restData[i].restaurant) {
      const result = await restaurantRating.findOne({
        ...
        where: {
          restaurantId: restData[i].restaurant?.id
        },
      });
      const averageRating = result.get('averageRating');
      let obj = {
        ...
        country: restData[i].restaurant?.country,
        ...
        openingTime: dateFormat(restData[i].restaurant?.openingTime),
        closingTime: dateFormat(restData[i].restaurant?.closingTime),
...
async function addToFav(req, res) {
  ...
  let check = await favouriteRestaurant.findOne({
    where: [{
      userId: userId
    }, {
      restaurantId: restaurantId
...
async function userFavRestaurantIds(req, res) {
  try {
    const { userId } = req.params;

    // Fetch favorite restaurants with null check
    const userRest = await favouriteRestaurant.findAll({
      where: { userId: userId },
      attributes: ['restaurantId'],
    });
    const ids = Array.isArray(userRest) && userRest.length > 0 
      ? userRest.map((dat) => dat?.restaurantId || null).filter((id) => id !== null) 
      : [];

    // Fetch wishlist with null check
    const userWishlist = await wishList.findAll({
      where: { userId: userId, status: true },
    });
    const wishListIds = Array.isArray(userWishlist) && userWishlist.length > 0 
      ? userWishlist.map((dd) => dd?.RPLinkId || null).filter((id) => id !== null) 
      : [];

    // Return response with both ids and wishListIds
    return res.json(
      ApiResponse("1", "Favourite Restaurant Ids", "", { ids, wishListIds })
    );
  } catch (error) {
    // Handle unexpected errors
    console.error("Error in userFavRestaurantIds:", error.message);
    return res.json(ApiResponse("0", "Error fetching data", error.message, {}));
  }
}
