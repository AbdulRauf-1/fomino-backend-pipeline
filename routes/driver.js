...
router.post('/changeCountry',upload.fields([
    { name:'licFrontPhoto', maxCount: 1},
    { name:'licBackPhoto', maxCount: 1}]), asyncMiddleware(driverController.changeCountry)
);
//4. Register step 4: Add address
router.post('/addAddress', asyncMiddleware(driverController.addAddress));
router.post('/driverOnline', asyncMiddleware(driverController.driverOnline));
router.post('/driverOffline', asyncMiddleware(driverController.driverOffline));
...
router.post('/phoneAuth', asyncMiddleware(driverController.phoneAuth));
router.post('/loginByPhoneNum', asyncMiddleware(driverController.loginByPhoneNum));
...
//11. logout  
router.post('/logout', validateToken ,asyncMiddleware(driverController.logout));
//Module 2:  Drawer 
//1. Get Profile
...
router.post('/foodPickedUp', asyncMiddleware(driverController.foodPickedUp));
...
router.post('/acceptInvitation', asyncMiddleware(driverController.acceptInvitation));
router.post('/cancelInviation', asyncMiddleware(driverController.cancelInvitation));
router.get('/getDriverRestaurants/:driverId', asyncMiddleware(driverController.getDriverRestaurants));
router.post('/getRestaurantCommissionDetails', asyncMiddleware(driverController.getRestaurantCommissionDetails));
router.post('/updateStatus', asyncMiddleware(driverController.updateStatus));
router.post('/updateRestaurantLink', asyncMiddleware(driverController.updateRestaurantLink));
router.post('/cancelContract', asyncMiddleware(driverController.cancelContract));
router.get('/getAllPayouts/:driverId', asyncMiddleware(driverController.getAllPayouts));
router.post('/addPayout', asyncMiddleware(driverController.addPayout));
router.post('/earningHistory', asyncMiddleware(driverController.earningHistory));
