...
router.post('/deleteAccount' , asyncMiddleware(userController.deleteAccount));
router.post('/home' , asyncMiddleware(userController.home1));
router.get('/home2/:lat/:lng/:cityName' , asyncMiddleware(userController.home2));
router.get('/home3' , asyncMiddleware(userController.home3));
router.post('/testhome' , asyncMiddleware(userController.testhome));
router.post('/payrexx_payment' , asyncMiddleware(userController.payrexx_payment));
router.get('/payrexx_design' , asyncMiddleware(userController.payrexx_design));
router.get('/get_all_active_payment_methods' , asyncMiddleware(userController.get_all_active_payment_methods));
router.get('/get_all_payment_providers' , asyncMiddleware(userController.get_all_payment_providers));
router.post('/orderAfterPayment' ,validateToken, asyncMiddleware(userController.orderAfterPayment));
...
router.post('/forgetPasswordRequestForRetailer', asyncMiddleware(userController.forgetPasswordRequestForRetailer));
...
router.post('/editAddress', validateToken, asyncMiddleware(userController.editAddress));
//3. Get all addresses 
...
//4. Get restaurants by ID Get Method for Web Pannel
router.get('/restaurantbyid', asyncMiddleware(userController.getRestaurantByIds));
//5. Get product by ID
...
router.get('/getprofile/:userId',  asyncMiddleware(userController.UsergetProfile));
//2. Update profile (name & phone number)
router.post('/updateprofile',upload.single("image"), asyncMiddleware(userController.updateProfile));
...
//5. Get restauant orders history for a user 
router.post('/orderdetailsfood' ,asyncMiddleware(userController.orderDetails));
router.get('/orderdetailsfood/:orderId', asyncMiddleware(userController.orderDetailsGet));
...
router.post('/afterPaymentGroupOrder',validateToken ,asyncMiddleware(userController.afterPaymentGroupOrder));
router.post('/joinGroup' ,asyncMiddleware(userController.joinGroup));
router.post('/joinMember' ,asyncMiddleware(userController.joinMember));
router.post('/notReady' ,asyncMiddleware(userController.notReady));
router.post('/groupOrderDetails' ,asyncMiddleware(userController.groupOrderDetails));
router.get('/groupOrderDetailsGet/:orderId' ,asyncMiddleware(userController.groupOrderDetailsGet));
...
router.post('/lockGroupOrder' ,asyncMiddleware(userController.lockGroupOrder));
router.post('/removeMember' ,asyncMiddleware(userController.removeMember));
router.post('/addAdditionalTip' ,asyncMiddleware(userController.addAdditionalTip));
router.post('/getOfferRestaurants' ,asyncMiddleware(userController.getOfferRestaurants));
router.post('/guestAccount' ,asyncMiddleware(userController.guestAccount));
router.patch('/updateLanguage', asyncMiddleware(userController.updateUserLanguage));
router.post('/translateMenu', asyncMiddleware(userController.translateMenu));
router.get('/getGroupOrders/:userId', asyncMiddleware(userController.getGroupOrders));
router.post('/sendDeleteAccountOTP', asyncMiddleware(userController.sendDeleteAccountOTP));
router.get('/testEmail', asyncMiddleware(userController.testEmail));
